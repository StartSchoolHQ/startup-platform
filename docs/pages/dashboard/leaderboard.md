...
- **Profile card** (2026-09-15): clicking the avatar/name cell on any student row (My Journey board, Team Journey members board, desktop and mobile) opens [`ProfileCardDialog`](../../../src/components/profile/profile-card-dialog.tsx) — a read-only ShadCN Dialog with avatar, name, active team, member-since, the four economy totals and the founder card texts with a lean badge. Fed by one RPC, `get_user_profile_card_v2(p_user_id)` (2026-09-15; SECURITY DEFINER because solo `task_progress` is owner-only under RLS — aggregates only, target must be active / self / caller admin; archived users come back as "not available"). Since My Journey credits are hidden from students, the My Journey block shows XP + "% done" (approved / active solo tasks) instead; v1 stays on the database, unused. No email is exposed. The dialog is standalone: any surface with a user id can reuse it by controlling `userId`.
...
- **Hooks:**
  ...
  - [`src/hooks/use-profile-card.ts`](../../../src/hooks/use-profile-card.ts) — `useProfileCard(userId)` → `get_user_profile_card_v2`
...
- **RPCs:**
  ...
  - `get_user_profile_card_v2(p_user_id)` — one student's profile card (dialog) incl. My Journey completion counts; `_v1` kept for rollback
