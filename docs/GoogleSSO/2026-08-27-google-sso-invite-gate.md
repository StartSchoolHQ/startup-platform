...
- Addendum 2026-09-14 (Elias): profile setup gained step 2, the **founder card** (`founder_profiles`, replaces deleted tasks MJ-P0-01/02); gate = name + avatar + card for every account. Elias's own admin account was purged from `public.*` to test a fresh signup (see CLAUDE.md rollback section). Develop preview URL `startup-platform-nine.vercel.app` is stale — test locally or on the real branch preview.
...
- Status 2026-09-15: hook + Google provider ENABLED in the dashboard on 2026-09-14 (gmail signup rejected, existing accounts link). Task 4 live test done via SQL/manual (not a Vitest file). Task 11: Elias's fresh Google signup went end to end (gate → setup → founder card → dashboard) on production; the E2E was run on production rather than the develop preview since no students exist yet. Task 9 deliberately skipped for now (6 unconfirmed accounts left in place). Task 12 (legacy invite paths) executed 2026-09-15 — 11 files deleted, docs trimmed; Task 13 (passwords) still open.
