...
export interface AgreementsQueueProps {
  title: string;
  description: string;
  /**
   * Hard whitelist: rows of any other agreement type never render on this
   * queue, regardless of the API response or the type filter. Also drives
   * the type dropdown options and the summary card counters.
   */
...
export function AgreementsQueue({
  ...
  // Whitelist the four statuses the board cares about AND this queue's
  // agreement types. Always applied, so a row in any other status or of a
  // type belonging to the other queue can never render, no matter what the
  // API returns. Pure view filter; rows stay in the DB.
  const visibleRows = useMemo(
    () =>
      rows.filter(
        (row) =>
          VISIBLE_STATUSES.has(row.status) && types.includes(row.agreement_type)
      ),
    [rows, types]
  );
  ...
  const typeOptions: Array<{ value: AgreementType | "all"; label: string }> = [
    ...
    <div className="flex-1 space-y-4 p-4 pt-6 md:p-8">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto">
          <AgreementsSummaryCard rows={visibleRows} types={types} />
        </div>
      </div>
