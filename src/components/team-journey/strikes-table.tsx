...
import { AlertTriangle, Zap, CreditCard, CheckCircle, Clock } from "lucide-react"
...
export function StrikesTable({ strikes }: StrikesTableProps) {
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200">
              <th className="text-left py-4 px-4 font-medium text-gray-600">Strike</th>
          ...
          <tbody>
            {strikes.map((strike, index) => {
              ...
              return (
                <tr key={strike.id} className={`${index < strikes.length - 1 ? 'border-b border-gray-100' : ''} hover:bg-gray-50`}>
                  <td className="py-4 px-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">{strike.datetime}</div>
                      </div>
                    </div>
                  </td>
                  <td className="py-4 px-4">
                  ...
                  <td className="py-4 px-4">
                    <div className="flex justify-end">
                      <Button size="sm" className={`text-xs ${actionConfig.class}`}>
                        {actionConfig.icon}
                        {actionConfig.text}
                      </Button>
                    </div>
