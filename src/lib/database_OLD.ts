...
// Admin: Get all strikes for admin panel
...
> {
  ...
  // Only show users with confirmed emails by joining with auth.users
  let query = supabase.from("users").select(`
      ...
      avatar_url, 
      graduation_level,
      auth_users:auth.users!inner(email_confirmed_at)
    `);

  // Filter: Only users with confirmed emails
  query = query.not("auth_users.email_confirmed_at", "is", null);
  ...
  const { data: rawUsers, error } = await query.limit(20);

  if (error) throw error;

  // Transform response to match expected interface (remove auth_users field)
  const users =
    (rawUsers as any)?.map(({ auth_users, ...user }: any) => user) || [];
 ...
 */
/**
 * OPTIMIZED: Get all team achievement dashboard data in single RPC call
  ...
  try {
    ...
    if (!recurringError && recurringData) {
      ...
      const recurringMap = new Map(
        recurringData.map((r: any) => [r.task_id, r])
      );
...
/**
 * @deprecated Use getTeamAchievementDashboard for better performance
...
export async function getTeamTasksVisible(teamId: string, userId?: string) {
  ...
  try {
    // OPTIMIZATION: Recurring task reset now handled by pg_cron every 30 minutes
    // No need to call on every page load - saves 500-1000 calls/day at scale
